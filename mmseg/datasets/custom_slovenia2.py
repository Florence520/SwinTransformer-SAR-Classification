...
class SloveniaDataset(Dataset):
    ...
    def __init__(
        ...
        self.img_dirs1 = img_dirs1
        self.img_dirs2 = img_dirs2
        ...
        if self.data_root is not None:
            if not osp.isabs(self.img_dirs1):
                self.img_dirs1 = osp.join(self.data_root, self.img_dirs1)
            if not osp.isabs(self.img_dirs2):
                self.img_dirs2 = osp.join(self.data_root, self.img_dirs2)
            if not (self.ann_dir is None or osp.isabs(self.ann_dir)):
                self.ann_dir = osp.join(self.data_root, self.ann_dir)

        # load annotations
        self.img_infos = self.load_annotations(self.img_dirs1, self.img_suffix, self.ann_dir, self.seg_map_suffix)

    def __len__(self):
        """Total number of samples of data."""
        return len(self.img_infos)

    def load_annotations(self, img_dirs1, img_suffix, ann_dir, seg_map_suffix):
        """Load annotation from directory. Note that we do not use split for slovenia dataset!

        Args:
            img_dirs1 (str): Path to image directory
        ...
        img_infos = []
        for img in mmcv.scandir(img_dirs1, img_suffix, recursive=True):
