...
class RandomCrop(BaseTransform):
    ...
    def crop_bbox(self, results: dict) -> tuple:
        ...
        img = results['img1']  # Use img1 to determine the crop bounding box
...
class PhotoMetricDistortion(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        """
        if results["img1"].ndim == 4:
            imgs1 = []
            for img in results["img1"]:
                ...
                if mode == 0:
                    img = self.contrast(img)
                imgs1.append(img)
            results["img1"] = np.stack(imgs1, axis=0)
        else:
            img = results['img1']
            # random brightness
            img = self.brightness(img)
        ...
        if results["img2"].ndim == 4:
            imgs2 = []
            for img in results["img2"]:
                ...
                if mode == 0:
                    img = self.contrast(img)
                imgs2.append(img)
            results["img2"] = np.stack(imgs2, axis=0)
        else:
            img = results['img2']
            # random brightness
            img = self.brightness(img)
...
class RandomFlip(MMCV_RandomFlip):
    ...
    def _flip(self, results: dict) -> None:
        ...
        # flip image
        if results["img1"].ndim == 4:
            imgs = [mmcv.imflip(img, direction=results["flip_direction"]) for img in results["img1"]]
            results["img1"] = np.stack(imgs, axis=0)
        else:
            results["img1"] = mmcv.imflip(results["img1"], direction=results["flip_direction"])

        if results["img2"].ndim == 4:
            imgs = [mmcv.imflip(img, direction=results["flip_direction"]) for img in results["img2"]]
            results["img2"] = np.stack(imgs, axis=0)
        else:
            results["img2"] = mmcv.imflip(results["img2"], direction=results["flip_direction"])

        img_shape = results['img1'].shape[1:3]
...
class Resize(BaseTransform):
    ...
    def _resize_img(self, results):
        """Resize images with ``results['scale']`` for both img1 and img2."""
        # 处理 img1
        if results["img1"].ndim == 4:  # 对于4维数据，D,H,W,C
            imgs1 = []
            if self.keep_ratio:
                for img in results["img1"]:
                ...
                h, w = results["img1"].shape[1:-1]  # T,H,W,C
                w_scale1 = new_w / w
                h_scale1 = new_h / h
            else:
                for img in results["img1"]:
                    ...
                    imgs1.append(tmp_img)
                w_scale1, h_scale1 = w_scale, h_scale
        else:
            if self.keep_ratio:
                img1, scale_factor = mmcv.imrescale(results["img1"], results["scale"], return_scale=True)
                new_h, new_w = img1.shape[:2]
                h, w = results["img1"].shape[:2]
                w_scale1 = new_w / w
                h_scale1 = new_h / h
            else:
                img1, w_scale1, h_scale1 = mmcv.imresize(results["img1"], results["scale"], return_scale=True)

                # 处理 img2
        if results["img2"].ndim == 4:  # 对于4维数据，D,H,W,C
            imgs2 = []
            if self.keep_ratio:
                for img in results["img2"]:
                    tmp_img, scale_factor = mmcv.imrescale(img, results["scale"], return_scale=True)
                ...
                new_h, new_w = tmp_img.shape[:2]
                h, w = results["img2"].shape[1:-1]  # T,H,W,C
                w_scale2 = new_w / w
                h_scale2 = new_h / h
            else:
                for img in results["img2"]:
                    tmp_img, w_scale, h_scale = mmcv.imresize(img, results["scale"], return_scale=True)
                    imgs2.append(tmp_img)
                w_scale2, h_scale2 = w_scale, h_scale
        else:
            if self.keep_ratio:
                img2, scale_factor = mmcv.imrescale(results["img2"], results["scale"], return_scale=True)
                new_h, new_w = img2.shape[:2]
                h, w = results["img2"].shape[:2]
                w_scale2 = new_w / w
                h_scale2 = new_h / h
            else:
                img2, w_scale2, h_scale2 = mmcv.imresize(results["img2"], results["scale"], return_scale=True)

                # 更新 scale_factor 和 keep_ratio
        scale_factor = np.array([w_scale1, h_scale1, w_scale1, h_scale1], dtype=np.float32)

        # 更新结果字典
        if results["img1"].ndim == 4:
            results["img1"] = np.stack(imgs1, axis=0)
            results["img1_shape"] = results["img1"].shape
        else:
            results["img1"] = img1
            results["img1_shape"] = img1.shape
        ...
        if results["img2"].ndim == 4:
            results["img2"] = np.stack(imgs2, axis=0)
            results["img2_shape"] = results["img2"].shape
        else:
            results["img2"] = img2
            results["img2_shape"] = img2.shape
...
class Normalize(BaseTransform):
    ...
    def __init__(self, mean1, std1, mean2, std2,to_rgb=True):
    ...
    def transform(self, results: dict) -> dict:
        ...
        #                                   self.to_rgb)
        if results["img1"].ndim == 4:
            imgs1 = [mmcv.imnormalize(img, self.mean1, self.std1, self.to_rgb) for img in results["img1"]]
            results["img1"] = np.stack(imgs1, axis=0)
        else:  # results['img'].ndim = 3
            results["img1"] = mmcv.imnormalize(results["img1"], self.mean1, self.std1, self.to_rgb)

        if results["img2"].ndim == 4:
            imgs2 = [mmcv.imnormalize(img, self.mean2, self.std2, self.to_rgb) for img in results["img2"]]
            results["img2"] = np.stack(imgs2, axis=0)
        else:  # results['img'].ndim = 3
            results["img2"] = mmcv.imnormalize(results["img2"], self.mean2, self.std2, self.to_rgb)

        results["img1_norm_cfg"] = dict(mean=self.mean1, std=self.std1, to_rgb=self.to_rgb)
        results["img2_norm_cfg"] = dict(mean=self.mean2, std=self.std2, to_rgb=self.to_rgb)
        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f"(mean1={self.mean1}, std1={self.std1},mean2={self.mean2}, std2={self.std2}, to_rgb=" f"{self.to_rgb})"
        return repr_str