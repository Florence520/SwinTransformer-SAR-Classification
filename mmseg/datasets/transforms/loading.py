...
class LoadDataFromFile(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        filename1 = results['img_p1_path']
        filename2 = results['img_p2_path']

        # t h w c for slovenia and brandenburg
        # t c h w for pastis-r
        if filename1.endswith(".mat"):  # for slovenia or brandenburg
            img1 = sio.loadmat(filename1)[self.field_name]
        ...
        elif filename1.endswith(".npz"):
            img1 = np.load(filename1)['s1']  # t c h w for pastis-r
            ...
            img1 = img1.transpose((3, 1, 2, 0))  # t h w c
            # img1 = img1.astype(np.float32)
        elif filename1.endswith(".npy"):
            img1 = np.load(filename1)  # t c h w for pastis-r
            # if 'pastis' in results['img_prefix'].lower(): # for pastis-r, t=[65,69,70,71] c=3 h=128 w=128
            #     # todo how to deal with t? note that we need t h w c!!! padding!!!
            #     if img.shape[0] != 71:
            #         num_padding = 71 - img.shape[0]
            #         img = np.pad(img, ((num_padding//2, num_padding - num_padding//2), (0,0), (0,0), (0,0)))
            #         assert img.shape[0] == 71
            img1 = img1.transpose((0, 2, 3, 1))  # t h w c
        else:
            raise NotImplementedError

        if filename2.endswith(".mat"):  # for slovenia or brandenburg
            img2 = sio.loadmat(filename2)[self.field_name]
            # if "slovenia" in results['img_prefix'].lower(): # for slovenia, t=12 h=500 w=500 c=2
            #     pass
            # elif 'brandenburg' in results['img_prefix'].lower(): # for brandenburg, t=41 h=224 w=224 c=2
            #     pass # todo====  how to deal with t=41?
        elif filename2.endswith(".npz"):
            img2 = np.load(filename2)['planet']  
            # img2 = np.stack([cv2.resize(img2[i], (64, 64)) for i in range(img2.shape[0])])
            img2 = resize(
                img2,
                (img2.shape[0], 64, 64, img2.shape[3]),
                mode='reflect'
            )
            ...
            # # 调整尺寸 (假设输入是 [C, H, W, D] -> 调整后 [C, 64, 64, D])
            # img2 = F.resize(img2, size=[64, 64])  # 自动批量处理
            img2 = img2.transpose((3, 1, 2, 0))  # t h w c
            # img2 = img2.cpu().numpy()
        elif filename2.endswith(".npy"):
            img2 = np.load(filename2)  # t c h w for pastis-r
            # if 'pastis' in results['img_prefix'].lower(): # for pastis-r, t=[65,69,70,71] c=3 h=128 w=128
            #     # todo how to deal with t? note that we need t h w c!!! padding!!!
            #     if img.shape[0] != 71:
            #         num_padding = 71 - img.shape[0]
            #         img = np.pad(img, ((num_padding//2, num_padding - num_padding//2), (0,0), (0,0), (0,0)))
            #         assert img.shape[0] == 71
            img2 = img2.transpose((0, 2, 3, 1))  # t h w c
        else:
            raise NotImplementedError
...
@TRANSFORMS.register_module()
class LoadTestDataFromFile(BaseTransform):
    """Load an data from .mat file.
...
class LoadAnnotations(MMCV_LoadAnnotations):
    ...
    def _load_seg_map(self, results: dict) -> None:
        ...
        if filename.endswith(".mat"):  # for slovenia or brandenburg
            ...
            gt_semantic_seg = np.load(filename)['truth']  # C=3,H,W
            gt_semantic_seg[gt_semantic_seg > 4] = 0
        elif filename.endswith(".npy"):
            gt_semantic_seg = np.load(filename)  # C=3,H,W
            gt_semantic_seg[gt_semantic_seg == 19] = 0
