...
class MulPastisrDataset(BaseSegDataset):
    METAINFO = dict(
        classes=(
            # 'no data',
            "Meadow",
    ...
    def load_data_list(self) -> List[dict]:
        ...
        for num in idx_list:
            img1 = "S1A_" + num + ".npy"
            img2 = "S2_" + num + ".npy"
            data_info = dict(img_p1_path=osp.join(img_p1_dir,img1), img_p2_path=osp.join(img_p2_dir,img2))
            if ann_dir is not None:
                seg_map = img1.replace('S1A', 'TARGET')
