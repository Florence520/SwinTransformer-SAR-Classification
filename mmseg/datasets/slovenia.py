...
class MulSloveniaDataset(BaseSegDataset):
    METAINFO = dict(
        classes=(
            # 'no data',
            "cultivated land",
    ...
    def load_data_list(self) -> List[dict]:
        ...
        for img1 in mmengine.scandir(img_p1_dir, self.img_suffix, recursive=True):
            data_info = dict(img_p1_path=osp.join(img_p1_dir,img1), img_p2_path=osp.join(img_p2_dir,img1))
            if ann_dir is not None:
                seg_map = img1.replace(self.img_suffix, self.seg_map_suffix)
