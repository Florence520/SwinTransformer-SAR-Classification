...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.decoder5 = UnetrUpBlock_(
            spatial_dims=spatial_dims,
            in_channels=16 * feature_size,
            out_channels=8 * feature_size,
            kernel_size=3,
            upsample_kernel_size=(1, 2, 2),  # todo 2->(1, 2, 2)
            norm_name=norm_name,
            res_block=True,
        )
        ...
        self.decoder3 = UnetrUpBlock(  # todo
            spatial_dims=spatial_dims,
    ...
    def forward(self, x_in1,x_in2):
        ...
        B,C,D,H,W = x_in1.shape
        x_in = F.pad(x_in1, (0, 0, 0, 0, 2, 2), mode='reflect')
        # if C == 2:
        #     input1_in = F.pad(x_in1, (0, 0, 0, 0, 2, 2), mode='reflect')
        #     # optical
        #     input2_in = F.pad(x_in2, (0, 0, 0, 0, 2, 2), mode='reflect')
        #     x_in = torch.cat((input1_in, input2_in), dim=1)
        # else:
        #     x_in = torch.cat((x_in1, x_in2), dim=1)
...
class UnetrUpBlock_(nn.Module):
    ...
    def __init__(
        ...
        # todo
        self.transp_conv = torch.nn.ConvTranspose3d(
            in_channels=in_channels, out_channels=out_channels, kernel_size=(3, 2, 2), stride=2, padding=(1, 0, 0)
        )
...
if __name__ == "__main__":
    x = torch.randn((2, 8, 16, 224, 224)).cuda()
    net = SwinUNETR(img_size=(16, 224, 224), in_channels=8, out_channels=8).cuda()
    out = net(x)
    print(out.shape)