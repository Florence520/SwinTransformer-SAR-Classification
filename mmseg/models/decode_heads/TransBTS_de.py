...
class TransformerBTS(BaseDecodeHead):
    def __init__(
        self,
        img_dim,
        patch_dim,
...
class BTS_de(TransformerBTS):
    def __init__(
        ...
        **kwargs
        ...
        self.endconv = nn.Conv3d(self.embedding_dim // 32, 18, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x1_1, x2_1, x3_1, encoder_output, intmd_encoder_outputs,auxillary_output_layers = x

        decoder_output = self.decode(
            x1_1, x2_1, x3_1, encoder_output, intmd_encoder_outputs, auxillary_output_layers
        )
        decoder_output = torch.mean(decoder_output, dim=2, keepdim=False)
