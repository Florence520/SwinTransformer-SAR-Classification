...
def cross_entropy(pred,
    ...
    # 确保 class_weight 是 GPU Tensor
    if class_weight is not None and not isinstance(class_weight, torch.Tensor):
        class_weight = torch.tensor(class_weight, device=label.device)
    ...
    if (avg_factor is None) and reduction == 'mean':
        if class_weight is None:
            ...
            label_weights = torch.zeros_like(label, dtype=torch.float32, device=label.device)
            label_weights[valid_mask] = class_weight[label[valid_mask]]  # 向量化索引
            # # 生成有效标签掩码
            # valid_mask = (label != ignore_index)
            # valid_labels = label[valid_mask]

            # # 初始化全零权重张量
            # label_weights = torch.zeros_like(label, dtype=torch.float32, device=class_weight.device)

            # # 仅对有效标签赋予class_weight
            # if valid_labels.numel() > 0:  # 确保有有效标签
            #     label_weights[valid_mask] = torch.stack(
            #         [class_weight[cls] for cls in valid_labels]
